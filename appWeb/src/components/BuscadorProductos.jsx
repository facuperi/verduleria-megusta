export const BuscadorProductos = ({
  ...
  return (
    <>
      <div className="bg-gray-800/50 p-4 rounded-lg shadow-sm border border-gray-700/50 mb-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          <button
            ...
            className={`px-3 py-1 rounded text-xs font-semibold border ${agregarComoNotaCredito ? 'bg-red-600 text-white border-red-600' : 'bg-gray-700 text-gray-300 border-gray-300 hover:bg-gray-200'}`}
          >
            ...
            {productosFiltrados.length === 0 ? (
              ...
              productosFiltrados.map(producto => (
                <div
                  ...
                  className="p-2 hover:bg-gray-700 cursor-pointer border-b flex justify-between items-center"
                >
                  <div>
                    <p className="font-semibold">{producto.nombre}</p>
                    <p className="text-sm text-gray-400">{producto.codigoInterno}</p>
                  </div>
      ...
      {sucursal && (
        <p className="text-sm text-gray-400 mb-2">Vendiendo en: <span className="font-semibold capitalize">{sucursal}</span></p>
      )}
        ...
        {productos
          ...
          .map(producto => (
            <div
              key={producto.id}
              onClick={() => onProductClick(producto)}
              className={`bg-gray-800/50 p-3 rounded-lg shadow-sm border border-gray-700/50 cursor-pointer hover:bg-gray-700 ${agregarComoNotaCredito ? 'ring-2 ring-red-400' : ''}`}
            >
              <h4 className="font-semibold text-sm">{producto.nombre}</h4>
              <p className="text-xs text-gray-400">{producto.codigoInterno}</p>
